  ...
  public class ArrayUtilities {
    ...
    public static function findMatchIndex(aArray:Array, oElement:Object, ...rest):Number {
      ...
      for (var i:Number = nStartingIndex; i < aArray.length; i++) {
		  compareElements(aArray[i], oElement);
        if(bPartialMatch) {
	...
	public static function compareElements(oArrayIndex:Object, oElement:Object):Boolean 
	{
		if (typeof(oArrayIndex) != "object") {
			return (oArrayIndex == oElement);
		}
		else {
			//THIS ONLY WORKS IF ELEMENTS TO COMPARE ARE OF THE SAME SIZE. AND ARRAYS TOO.
			if (oArrayIndex[0] == oElement[0] && oArrayIndex[1] == oElement[1]) return true;
			return false
		}
	}
    ...
    public static function duplicate(oArray:Object, bRecursive:Boolean = false):Object {
      ...
      if(bRecursive) {
        ...
        if(oArray is Array) {
          ...
          oDuplicate = new Object();
          for(var s_Item:String in oArray) {
            oDuplicate[s_Item] = oArray[s_Item];
          }
          return oDuplicate;
        }
      }
